...
class TestCalculateAveragePricePerCountryUseCase:
    ...
    def test_should_calculate_correctly(
        ...
        result: Result[AveragePricePerCountryViewModel, StatisticsFailure] = self._use_case.execute(entries)

        assert result.is_ok()

        ok_result: Ok[AveragePricePerCountryViewModel, StatisticsFailure] = typing.cast(
            Ok[AveragePricePerCountryViewModel, StatisticsFailure],
            result
        )
